...
lShape = [[(gameBoardWidth / 2) - 1, 1], [(gameBoardWidth / 2) - 1, 0], [(gameBoardWidth / 2) - 1, 2],
          [gameBoardWidth / 2, 2]]
mlShape = [[(gameBoardWidth / 2), 1], [(gameBoardWidth / 2), 0], [(gameBoardWidth / 2), 2],
...
class Shape():
    ...
    def moveLeft(self):
        blocked = False
        for i in range(self.numblocks):
            if self.blocklist[i].gridxpos == 0 or activeBoardSpot[self.blocklist[i].gridxpos - 1][
                self.blocklist[i].gridypos] == True:
                blocked = True
        if blocked == False:
            for i in range(self.numblocks):
    ...
    def moveRight(self):
        blocked = False
        for i in range(self.numblocks):
            if self.blocklist[i].gridxpos == gameBoardWidth - 1 or activeBoardSpot[self.blocklist[i].gridxpos + 1][
                self.blocklist[i].gridypos] == True:
                blocked = True
        if blocked == False:
            for i in range(self.numblocks):
    ...
    def moveDown(self):
        blocked = False
        for i in range(self.numblocks):
            if self.blocklist[i].gridypos == gameBoardHeight - 1 or activeBoardSpot[self.blocklist[i].gridxpos][
                self.blocklist[i].gridypos + 1] == True:
                blocked = True
        if blocked == False:
            for i in range(self.numblocks):
